...
typedef struct {
    int cmd;                /*<! 特定的LCD命令 */
    const void *data;       /*<! 保存命令特定数据的缓冲区 */
    size_t data_bytes;      /*<! 内存中“数据”的大小，以字节为单位 */
    unsigned int delay_ms;  /*<! 此命令后的延迟（以毫秒为单位） */
} ili9341_lcd_init_cmd_t;

/**
 * @brief LCD panel vendor configuration.
 *
 * @note  此结构需要传递到esp_lcd_panel_dev_config_t中的vendor_config字段.
 *
 */
typedef struct {
